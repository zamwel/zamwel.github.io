...
var headings = ['Zero', 'One', 'Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten'];
...
if (elements !== null) {
    readTextFile("/php/full.json", function (text) {
      ...
      <td id="buy" class="bg-danger text-light price">$${(element['price'])/2} &nbsp; BUY</td>
      <!-- <td id="buy" class="bg-danger text-light price">$25 &nbsp; BUY</td>-->
       </tr>
       `;
            html += la;
        });
...
function createUser(value, row_num) {
    auth.onAuthStateChanged(user => {
        ...
        db.collection('unicc').doc(email).set({
...
auth.onAuthStateChanged(user => {
    ...
    db.collection("unipur").where("useremail", "==", email).get().then((v) => {
        ...
        v.forEach((e) => {
            ...
            <div class="card-header" id="heading${headings[i]}">
              <h5 class="mb-0">
                <button id="title${headings[i]}" class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapse${headings[i]}"
            ...
            <div id="collapse${headings[i]}" class="collapse" aria-labelledby="heading${headings[i]}" data-parent="#accordionExample">
             ...
             &nbsp;&nbsp;<strong>Cvv</strong>:  ${e.data()['cvv']}<br>              
    ...
    document.getElementById("dload").addEventListener('click', (e) => {
        ...
        $('#accordionExample').on('click', '.card', function () {
            ...
            if (count == 1) {
                ...
                content = document.getElementById(`ccbody${headings[index]}`).innerText.replace('Download', "https://uniccs.cc");
                conttext = document.getElementById(`title${headings[index]}`).innerText;
